    ...
    <template v-if="musicDetail">
      ...
      <div class="flex gap-6">
        <div class="relative h-48 w-48 flex-shrink-0 overflow-hidden rounded-lg bg-muted">
          <img
            v-if="musicDetail.coverUrl"
            :src="musicDetail.coverUrl"
            :alt="musicDetail.name"
            class="h-full w-full object-cover"
          />
          <div
            v-else
            class="flex h-full w-full items-center justify-center bg-gradient-to-br from-red-400 to-red-600 text-white"
          >
        ...
        <div class="flex-1 space-y-4">
          <div>
            ...
            <p class="mt-2 text-sm text-muted-foreground">
              时长：{{ formatDuration(musicDetail.duration) }}
            </p>
            <p class="text-sm text-muted-foreground">
              播放量：{{ formatPlayCount(musicDetail.playCount) }}
            </p>
          </div>
          ...
          <div v-if="musicDetail.artists && musicDetail.artists.length > 0" class="space-y-2">
            ...
            <div class="flex flex-wrap gap-2">
              <div
                v-for="artist in musicDetail.artists"
                :key="artist.id"
                class="flex items-center gap-2 rounded-full bg-muted px-3 py-1.5"
              >
                <img

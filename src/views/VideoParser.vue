				...
				<Card v-else-if="parseResult" class="overflow-hidden border border-primary/20">
					...
					<div class="p-6">
						...
						<div class="mt-6 flex flex-col sm:flex-row gap-3">
							<!-- 立即播放 -->
							<Button v-if="canPlayAudio" variant="default" size="lg" class="flex-1 h-11" @click="handlePlay" :disabled="isActionLoading">
...
const canPlayAudio = computed(() => {
	return Boolean(parseResult.value?.audioSupported) && hasAvailableAction('AUDIO_STREAM') && canUseVideoStreamPlayback(parseResult.value)
})
